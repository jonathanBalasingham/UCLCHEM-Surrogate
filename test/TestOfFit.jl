...
ESN.train!(esn, [X], [y], 1e-11)
ESN.train!(desn, [X], [y], 2e-7)

pred1 = ESN.predict!(esn, X) |> x->vcat(x, hcat(sum.(eachcol(x))...))
pred2 = ESN.predict!(desn, X) |> x->vcat(x, hcat(sum.(eachcol(x))...))
_y = y |> x->vcat(x, hcat(sum.(eachcol(x))...))

plot(sol.t[2:end], _y', xscale=:log10, label="GT", layout=4, legend=:outertopright)
plot!(sol.t[2:end], pred1', xscale=:log10, label="CTESN", layout=4)
plot!(sol.t[2:end], pred2', xscale=:log10, label="DeepCTESN", layout=4)

savefig(projectdir("images", "test_of_fit_RP_ESR.png"))

# SCR
esn = ESN.EchoStateNetwork{Float64, ESN.SimpleCycleReservoir{Float64}}(3,300,3; c=.7);
desn = ESN.DeepEchoStateNetwork{Float64, ESN.SimpleCycleReservoir{Float64}}(3, 50, 6, 3, c=.5);

ESN.train!(esn, [X], [y], 1e-11)
ESN.train!(desn, [X], [y], 1e-15)
...
savefig(projectdir("images", "test_of_fit_RP_SCR.png"))

# DLR
esn = ESN.EchoStateNetwork{Float64, ESN.DelayLineReservoir{Float64}}(3,300,3, c=.7);
desn = ESN.DeepEchoStateNetwork{Float64, ESN.DelayLineReservoir{Float64}}(3, 50, 6,3, c=.5);

ESN.train!(esn, [X], [y], 1e-11)
ESN.train!(desn, [X], [y], 1e-12)
...
savefig(projectdir("images", "test_of_fit_RP_DLR.png"))
...
savefig(projectdir("images", "test_of_fit_RP_DLRF.png"))