...
physical_parameters = Parameters(rates_set_upper_bound...)
cnp = formulate_all(rfp, icfp,physical_parameters, tspan=tspan)
prob = ODEProblem(cnp.network, cnp.u0 ./ sum(cnp.u0), cnp.tspan)
...
function test!(esn, beta)
  ...
  pred = ESN.predict!(esn, warmup, steps) |> x->vcat(x, hcat(sum.(eachcol(x))...))
  Flux.Losses.mae(pred, _y[:, warmup_length:end])
end
...
desn = ESN.DeepEchoStateNetwork{Float64, ESN.EchoStateReservoir{Float64}}(input_dimension, 85, 6,output_dimension);
...
desn = ESN.DeepEchoStateNetwork{Float64, ESN.SimpleCycleReservoir{Float64}}(input_dimension, 85, 6, output_dimension, c=.65);

test_all(esn)
test_all(desn)

# DLR
esn = ESN.EchoStateNetwork{Float64, ESN.DelayLineReservoir{Float64}}(input_dimension,500, output_dimension, c=.8);
desn = ESN.DeepEchoStateNetwork{Float64, ESN.DelayLineReservoir{Float64}}(input_dimension, 85, 6, output_dimension, c=.6);

test_all(esn)
test_all(desn)

# DLR w/F
esn = ESN.EchoStateNetwork{Float64, ESN.DelayLineReservoir{Float64}}(input_dimension,500,output_dimension, c=.7, feedback=-.1);
desn = ESN.DeepEchoStateNetwork{Float64, ESN.DelayLineReservoir{Float64}}(input_dimension, 85, 6, output_dimension, c=.7, feedback=-.1);
